...
const fetchHrLeaveList = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/hr-leave-status/`);
  return data || [];
};
...
const ManagerHRLeave = () => {
  ...
  const { data: hrLeaveList = [], isFetching, isError } = useQuery({
    queryKey: ["hrLeaves"],
    queryFn: fetchHrLeaveList,
    placeholderData: [],
    staleTime: 5000,
  });
  ...
  const handleStatusChange = async (status, leaveId) => {
    ...
    try {
      const response = await axios.post(`${apiBaseUrl}/hr-leave-status/`, {
        leave_id: leaveId,
        status: status.toLowerCase(),
      });
      toast.success(response.data.message);
      queryClient.invalidateQueries(["hrLeaves"]);
    } catch (error) {
      ...
      {isFetching ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">HR Leave Records</h5>
              <p className="text-gray-500 text-sm">Manage leave requests for HR</p>
            </div>
            <Button
              onClick={() => queryClient.invalidateQueries(["hrLeaves"])}
          ...
          <div className="border rounded-md bg-white">
            <Table className="table-auto">
              ...
              <TableBody>
                {hrLeaveList.length > 0 ? (
                  hrLeaveList.map((leave, index) => (
