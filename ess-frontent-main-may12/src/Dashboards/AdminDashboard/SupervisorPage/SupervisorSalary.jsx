...
const fetchSalaryList = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/supervisor-salary/history/`);
  return data || [];
};
...
const SupervisorSalary = () => {
  ...
  const handleDelete = async (salary) => {
    if (window.confirm(`Are you sure you want to delete Salary ID ${salary.id}?`)) {
      try {
        await axios.delete(`${apiBaseUrl}/delete-supervisor-salary/${salary.id}/`);
        toast.success(`Salary ID ${salary.id} deleted successfully.`);
        queryClient.invalidateQueries(["supervisorSalaries"]);
      } catch (error) {
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingSalaries || isFetchingSupervisors) ? (
        <SkeletonLoading />
      ) : (isErrorSalaries || isErrorSupervisors) ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Supervisor Salary Records</h5>
              <p className="text-gray-500 text-sm">Manage salary details for supervisors</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <Button
                onClick={() => queryClient.invalidateQueries(["supervisorSalaries"])}
                  ...
                  salaryList.map((salary, index) => (
                    <TableRow key={salary.id}>
                      ...
                      <TableCell className="text-base">
                        {supervisorList.find((s) => s.id === salary.user_id)?.supervisor_name || salary.user_id}
                      </TableCell>
