...
const fetchAttendanceRecords = async (supervisorId) => {
  ...
  const { data } = await axios.get(`${apiBaseUrl}/admin/supervisor-attendance-history/`, {
...
const fetchAllAttendanceRecords = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/admin_all_supervisors_attendance_history`);
...
const AttendanceCard = ({ record, index }) => (
  <Card className="shadow-sm">
    ...
    <CardContent className="space-y-2">
      ...
      <p><strong>In Status:</strong> {record.in_status || "-"}</p>
      <p><strong>Out Status:</strong> {record.out_status || "-"}</p>
...
const SupervisorAttendanceRecords = ({ supervisorId }) => {
  ...
  const { data: attendanceRecords = [], isFetching: isFetchingAttendance, isError: isErrorAttendance } = useQuery({
    queryKey: ["supervisor_attendance", supervisorId],
    queryFn: () => fetchAttendanceRecords(supervisorId),
    placeholderData: [],
    staleTime: 5000,
  ...
  const { data: allAttendanceRecords = [], isFetching: isFetchingAllAttendance } = useQuery({
    queryKey: ["all_supervisor_attendance"],
  ...
  const attendanceData = supervisorId ? attendanceRecords : allAttendanceRecords;
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (record) =>
          record.supervisor_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                      ...
                      <TableHeader>
                        <TableRow className="text-base bg-slate-100">
                          ...
                          <TableCell onClick={() => handleSort("supervisor_name")} className="cursor-pointer">
                            Supervisor Name {getSortIcon("supervisor_name")}
                          </TableCell>
                          ...
                          <TableCell onClick={() => handleSort("in_status")} className="cursor-pointer">
                            In Status {getSortIcon("in_status")}
                          </TableCell>
                          <TableCell onClick={() => handleSort("out_status")} className="cursor-pointer">
                            Out Status {getSortIcon("out_status")}
                          </TableCell>
...
const SupervisorAttendance = () => {
  ...
  const { data: supervisors = [], isFetching: isFetchingSupervisors, isError: isErrorSupervisors } = useQuery({
  ...
  const handleRefresh = () => {
    ...
    queryClient.invalidateQueries(["supervisor_attendance"]);
    queryClient.invalidateQueries(["all_supervisor_attendance"]);
  };
    ...
    <div className="p-2 sm:p-4 min-h-screen">
      {isFetchingSupervisors ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Supervisor Attendance</h5>
              <p className="text-gray-500 text-sm">Select a supervisor to view attendance records</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <select
