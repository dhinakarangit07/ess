...
const SupervisorLeavePolicies = () => {
  ...
  const { data: leavePolicies = [], isFetching: isFetchingPolicies, isError: isErrorPolicies } = useQuery({
    queryKey: ["supervisorLeavePolicies"],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiBaseUrl}/edit_supervisor_leave_balance/${selectedLeaveId}/`, formData);
      ...
      queryClient.invalidateQueries(["supervisorLeavePolicies"]);
      setShowModal(false);
    } catch (error) {
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingSupervisors || isFetchingPolicies) ? (
        <SkeletonLoading />
      ) : (isErrorSupervisors || isErrorPolicies) ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Supervisor Leave Policies</h5>
              <p className="text-gray-500 text-sm">Manage leave balances for supervisors</p>
            </div>
            <div className="mt-2 md:mt-0 flex gap-3">
              <Button
                onClick={() => queryClient.invalidateQueries(["supervisorLeavePolicies"])}
            ...
            <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white p-6 rounded-lg w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {!isEditMode && (
                    <div>
                      <label className="text-sm text-gray-600">Select Supervisor</label>
                      <select
