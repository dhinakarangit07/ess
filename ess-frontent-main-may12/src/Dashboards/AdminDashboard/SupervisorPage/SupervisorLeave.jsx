...
const fetchSupervisorLeaveList = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/supervisor-leave-status/`, {
...
const SupervisorLeave = () => {
  ...
  const { data: supervisorLeaveList = [], isFetching, isError } = useQuery({
    queryKey: ["supervisorLeaves"],
    queryFn: fetchSupervisorLeaveList,
    placeholderData: [],
    staleTime: 5000,
  });
  ...
  const handleStatusChange = async (status, leaveId) => {
    ...
    try {
      ...
      queryClient.invalidateQueries(["supervisorLeaves"]);
    } catch (error) {
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {isFetching ? (
        ...
        <div className="bg-white rounded-lg shadow-sm.ConcurrentHashMap p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Supervisor Leave Records</h5>
              <p className="text-gray-500 text-sm">
                Manage leave requests for supervisors
              </p>
            </div>
            <Button
              onClick={() => queryClient.invalidateQueries(["supervisorLeaves"])}
                    ...
                    <TableRow key={leave.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button
                            onClick={() =>
                              handleStatusChange("Approved", leave.id)
                            }
                            disabled={
                              leave.status.toLowerCase() === "approved" ||
                          ...
                          <Button
                            onClick={() =>
                              handleStatusChange("Rejected", leave.id)
                            }
                            disabled={
                              leave.status.toLowerCase() === "approved" ||
                              leave.status.toLowerCase() === "rejected" ||
