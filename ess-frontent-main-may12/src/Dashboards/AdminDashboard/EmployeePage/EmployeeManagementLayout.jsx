...
const TopNav = () => {
  const navItems = [
    { to: ".", label: "Employee", icon: <Users className="w-5 h-5" />, end: true },
    { to: "attendance", label: "Attendance", icon: <UserCheck className="w-5 h-5" /> },
    { to: "attendance-reset", label: "Attendance Reset", icon: <UserCog className="w-5 h-5" /> },
    { to: "Chart", label: "Statistics", icon: <ChartArea className="w-5 h-5" /> },
    { to: "employee-leave-policies", label: "Employee Leave Policies", icon: <Calendar className="w-5 h-5" /> },
  ...
  return (
    <nav className="bg-white shadow-sm">
      <div className="flex items-center justify-between p-4 border-b">
        <h2 className="text-lg font-semibold text-gray-800">Employee Dashboard</h2>
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-600">Employee Admin</span>
          <Button
            variant="ghost"
