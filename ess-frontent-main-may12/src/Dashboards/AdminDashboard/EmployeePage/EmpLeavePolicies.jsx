...
const EmpLeavePolicies = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${apiBaseUrl}/edit_employee_leave_balance/${selectedLeaveId}/`, formData);
        toast.success("Leave balance updated successfully.");
      } else {
        await axios.post(`${apiBaseUrl}/leave-balance/update/${formData.selectedEmployee}/`, formData);
  ...
  return (
    <div className="p-2 sm:p-4 min-h-screen">
      {(isFetchingEmployees || isFetchingPolicies) ? (
        <SkeletonLoading />
      ) : (isErrorEmployees || isErrorPolicies) ? (
