...
const fetchEmployeeLeaveList = async () => {
  const { data } = await axios.get(`${apiBaseUrl}/employee-leave-status/`);
  return data || [];
};
...
const EmpLeave = () => {
  ...
  const { data: employeeLeaveList = [], isFetching, isError } = useQuery({
    queryKey: ["employeeLeaves"],
    queryFn: fetchEmployeeLeaveList,
    placeholderData: [],
    staleTime: 5000,
  });
  ...
  const handleStatusChange = async (status, leaveId) => {
    ...
    try {
      const response = await axios.post(`${apiBaseUrl}/employee-leave-status/`, {
        leave_id: leaveId,
  ...
  const handleDelete = async (leaveId) => {
    if (window.confirm(`Are you sure you want to delete Leave ID ${leaveId}?`)) {
      setIsProcessing(leaveId);
      try {
        await axios.delete(`${apiBaseUrl}/delete_employee_leave/${leaveId}/`);
      ...
      {isFetching ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex flex-col md:flex-row justify-between mb-4">
            <div>
              <h5 className="font-semibold text-lg">Employee Leave Records</h5>
              <p className="text-gray-500 text-sm">Manage leave requests for employees</p>
            </div>
            <Button
              onClick={() => queryClient.invalidateQueries(["employeeLeaves"])}
            ...
            <Table className="table-auto">
              ...
              <TableBody>
                {employeeLeaveList.length > 0 ? (
                  employeeLeaveList.map((leave, index) => (
